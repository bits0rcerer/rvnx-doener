...
<div class="flex items-center">
	{#each [...Array(maxStars).keys()] as i}
		<svg
			...
			class:text-gray-300={(hovering ? hoverRating : rating) < i + 1 || unrated}
			fill="currentColor"
			on:mouseenter={editable ? () => (hoverRating = i + 1) : null}
			on:mouseup={editable
				? () => {
						rating = i + 1;
						if (ratingChangedCallback) ratingChangedCallback(rating);
				  }
				: null}
			viewBox="0 0 20 20"
	...
	<span class="ml-auto text-sm font-medium text-gray-500 dark:text-gray-400"
		>{unrated ? '- /' : Math.min(maxStars, Math.max(0, rating)) + ' /'} {maxStars}</span
	>
</div>
