<script>
    ...
    async function searchAddress(search) {
        if (search && search.length > 3) {
            const res = await fetch("https://eu1.locationiq.com/v1/search?format=json&key=" + apiKey + "&q=" + search)
                .then(resp => resp.json())
    ...
    function submitForm(e) {
        ...
        if (!selectedShopName || !selectedShopName.trim() || !selectedAddress)
            return
    ...
    function displayCords(value) {
        ...
        if (value && value.lat && value.lon) {
            map.setView(
                [value.lat, value.lon], map.getZoom(), {
                    renderer: leaflet.svg()
                }
            );
...
<div class="flex flex-col mx-auto container h-screen">
    <div class="card w-full h-1/3 md:h-1/2 xl:rounded-2xl xl:my-2" id="previewMap"></div>
    <div class="card p-4 xl:rounded-2xl">
        ...
        <form class="mt-2">
            <div class="mb-6">
                ...
                <input bind:value={selectedShopName}
            ...
            <div class="flex items-start mb-6">
                <div class="flex items-center h-5">
                    <input bind:checked={anonymous}
                           class="w-4 h-4 bg-gray-50 rounded border border-gray-300 focus:ring-3 focus:ring-blue-300 dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600 dark:ring-offset-gray-800"
                           id="anonymous"
                           required
                           type="checkbox">
                </div>
                <label class="ml-2 text-sm font-medium text-gray-900 dark:text-gray-300" for="anonymous">Anonym
                    eintragen</label>
            </div>
            <button class="mt-2 text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm sm:w-auto px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
                    on:click={submitForm}>
                Eintragen
            </button>
        </form>
